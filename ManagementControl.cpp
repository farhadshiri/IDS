...
int ManagementControl::StartRealTimeIDS()
{
    if ( NeuralNetObj.InitializeNet(true) && ResponderObj.Initialize("Real Time"))
    {
        ...
        {
            printf("LogCorrelation: Real-Time LogCorrelation based Intrusion Detector\n Started Successfully...\n");
            return 1;
        }
    }
    ...
    perror ("LogCorrelation Error: Can not Start Real-Time LogCorrelation based Intrusion Detector!\n");
    return 0;
...
int ManagementControl::StartOffLineIDS()
{
    ...
    */
    ...
    {
        printf("LogCorrelation: Off-Line LogCorrelation based Intrusion Detector\n started successfully.\nPlease Wait....\n");
        retval=TcpDumpPacketProviderObj.StartSniffer(false);
        NeuralNetObj.FreeNet();
        ResponderObj.Destroy();
    } else
    {
        perror ("LogCorrelation Error: Can not initialize Neural Net based Analyzer and Responder components !\n");
        return -1;	
    }

    if (!retval)
    {
        perror ("LogCorrelation Error: Can not start Off-line LogCorrelation based Intrusion Detector !\n");
        return 0;
    }
	
    printf("LogCorrelation: Off-line LogCorrelation based Intrusion Detection terminated successfully.\n");
    return 1;
...
int ManagementControl::StopRealTimeIDS()
{
    ...
    {
        perror ("LogCorrelation Error: Can not Correct Stop Real-Time LogCorrelation based Intrusion Detector!\n");
        return 0;
    }
    IntrusionDetectionThreadObj.terminate();
    IntrusionDetectionThreadObj.wait();
...
int ManagementControl::StartCapturePackets(char *ParamString)
{
    ...
    {
        printf("LogCorrelation: Capturing Network Packets started successfully ...\n");
        return 1;
    }
    
    CapturePacketsThreadObj.terminate();
    CapturePacketsThreadObj.wait();
    perror ("LogCorrelation Error: Can not start Capturing Network Packets !\n");
    return 0;
...
int ManagementControl::StopCapturePackets()
{
    CapturePacketsThreadObj.terminate();
    CapturePacketsThreadObj.wait();
    printf("LogCorrelation: Capturing Network Packets stoped successfully.\n");
    return 1;    
